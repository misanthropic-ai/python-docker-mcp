...
async def test_execute_transient(mock_session):
    ...
    mock_session.call_tool.assert_called_once_with(
        "execute-transient", {"code": "print('Hello, World!')"}
    )
...
async def test_execute_transient_with_state(mock_session):
    ...
    # Verify the call
    mock_session.call_tool.assert_called_once_with(
        "execute-transient", {"code": "print(f'The value is {state[\"value\"]}')", "state": state}
    )
...
async def test_execute_transient_with_error(mock_session):
    ...
    # Verify the call
    mock_session.call_tool.assert_called_once_with(
        "execute-transient", {"code": "print(undefined)"}
    )
...
async def test_execute_persistent_new_session(mock_session):
    ...
    # Verify the call
    mock_session.call_tool.assert_called_once_with(
        "execute-persistent", {"code": "print('Hello from persistent container')"}
    )
...
async def test_execute_persistent_existing_session(mock_session):
    ...
    result, session_id = await client.execute_persistent(
        "print('Using existing session')", "abc-123"
    )
    
    # Verify the call
    mock_session.call_tool.assert_called_once_with(
        "execute-persistent", {"code": "print('Using existing session')", "session_id": "abc-123"}
    )
...
async def test_install_package(mock_session):
    ...
    # Verify the call
    mock_session.call_tool.assert_called_once_with(
        "install-package", {"package_name": "numpy"}
    )
...
async def test_install_package_with_session(mock_session):
    ...
    # Call the method with session ID
    result = await client.install_package("pandas", "abc-123")
    
    # Verify the call
    mock_session.call_tool.assert_called_once_with(
        "install-package", {"package_name": "pandas", "session_id": "abc-123"}
    )
...
async def test_cleanup_session(mock_session):
    ...
    text_content = MagicMock(
        type="text",
        text="Session abc-123 cleaned up successfully"
    )
    response.content = [text_content]
    mock_session.call_tool.return_value = response
    ...
    # Verify the call
    mock_session.call_tool.assert_called_once_with(
        "cleanup-session", {"session_id": "abc-123"}
    )
